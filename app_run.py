...
def narrate_dataset(df_: pd.DataFrame) -> str:
    ...
    base = (
        ...
        f"- Accounts: {n_accts} | Merchants: {n_merch}\n"
...
def parse_time_window(q: str) -> Optional[Tuple[date, date]]:
    ql = _normalize(q)
    if "yesterday" in ql: return today_window(TODAY - timedelta(days=1))
    ...
    for token in MONTHS:
        if token in ql:
            ...
            my = re.search(rf"(\d{{4}})\s+{token}", ql)
            if my:
...
def handle_rank_query(q: str, frame: pd.DataFrame) -> None:
    ...
    sel = agg_sorted.head(n) if ascending else agg_sorted.head(n)  # head works for both after sorting
    ...
    fig = px.bar(
        sel,
        x=dim_label, y=pretty_metric_name(metric),
        title=title
    )
...
def handle_compare_by_query(q: str, frame: pd.DataFrame) -> None:
    ...
    agg_sorted = agg.sort_values("value", ascending=False).head(10)
    out = agg_sorted.rename(columns={dim_col: dim_label, "value": pretty_metric_name(metric)})
    ...
    fig = px.bar(out, x=dim_label, y=pretty_metric_name(metric), title=title)
    fig.update_layout(xaxis_title=dim_label, yaxis_title=pretty_metric_name(metric))
...
if user_q:
    ...
    with st.chat_message("assistant"):
        ...
        # D) Fallback → agent or Not Found (uses dynamic df.columns in prompt)
        else:
            if build_err:
...
# ------------------------------------------------------------------------------
# 10) Proactive suggestion buttons (simple)
# ------------------------------------------------------------------------------
def _suggestions() -> List[str]:
    if df is None or df.empty:
        return ["Show schema", "Top merchants by spend", "Monthly spend trend"]
    return [
        "Show schema",
        "Which are the top 5 accounts this month?",
        "Total spend in last 3 complete months",
        "List merchants with highest spend this year",
    ][:4]
